...
newtype Metric a = Metric { unMetric :: Glue a }
...
type GenericRegistrable f = ( Generic (f Metric)
                            , Generic (f Identity)
                            , GRegistrable (Rep (f Metric)) (Rep (f Identity))
                            )

genericRegister
  :: ( GenericRegistrable f
     )
  => f Metric
  -> IO (f Identity)
...
instance (GRegistrable i o, GRegistrable j p) => GRegistrable (i :+: j) (o :+: p) where
...
type GenericExportable f = ( Generic (f Identity)
                           , GExportable (Rep (f Identity))
                           )
...
data Template = Template Info LByteString [Sample]
                deriving Show
