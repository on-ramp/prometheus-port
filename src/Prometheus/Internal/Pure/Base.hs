...
newtype Pure f s = Pure { unPure :: NoIdentity f s }
...
class Extract a ~ e => PureExtractable e a where
  pureExtract :: a -> e
...
data Sample = DoubleSample LByteString Tags Double
...
class Label l => PureMappable l a b | a -> b where
  pmap :: l -> b -> (b -> b) -> a -> a

instance ( Label l
         ) => PureMappable l (Pure (Map l) b) b where
